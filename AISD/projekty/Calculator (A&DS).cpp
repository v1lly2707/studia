...
/*char* GetNextToken(struct Node** top)
{
...
/*int ResultCalculation(struct Stack* _Stack, char op)
{
    Node node1 = PopFromTheStack(_Stack);
    Node node2 = PopFromTheStack(_Stack);

    if (_Stack->top == nullptr || node1.next == nullptr || node2.next == nullptr)
    {
        cout << "Stack is empty!\n";
        return -1;
    }

    int operand1 = atoi(node1.sequence);
    int operand2 = atoi(node2.sequence);
    ...
    sprintf_s(result_str, "%d", result);
    PushToTheStack(_Stack, result_str);


    //return result;
}*/

/*void DisplayResult(struct Stack* _Stack)
{
    if (_Stack->top == nullptr)
        cout << "Stack is empty!\n";
...
int main()
{
    ...
    while (n--)
    {
        ...
        /*while (_List.head != nullptr)
        {
            char* Token = GetNextToken(&_List.head);


            if (DetectOperand(Token[0]))
                PushToTheStack(&_Stack, Token);
            else
                int result = ResultCalculation(&_Stack, Token[0]);
