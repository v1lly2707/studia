...
def training(model, x, y):
    ...
    prepared = np.arange(x.shape[0])
    np.random.shuffle(prepared)
    start = 0

    chosen = np.empty(minibatch_size)
    ...
    for _ in range(n_steps):
        ...
        chosen = torch.randint(0, x.shape[0], (minibatch_size,))
        #first = prepared[start:minibatch_size]
        #start += minibatch_size

        #second = np.empty(0)

        #if first.size < minibatch_size:
         #   np.random
          #  nAppend = minibatch_size - first.size
           # second = prepared[:nAppend]
            #start = nAppend

        #chosen[:first.size] = first
        #chosen[first.size:] = second
...
if __name__ == '__main__':
    ...
    student_id = 197725         # Twój numer indeksu, np. 102247
