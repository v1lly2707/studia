...
%       z zastosowaniem zmiennych vpa.
    ...
    dates = energy_2025.Poland.Wind.Dates; % TODO
    y = energy_2025.Poland.Wind.EnergyProduction; % TODO
    ...
    % Wykres 1: ya
    subplot(3,1,1);
    plot(x_fine, ya, 'b-', x, y, 'ko');
    title('Aproksymacja w precyzji double');
    xlabel('x (znormalizowany czas)');
    ylabel('Produkcja energii');
    legend('aproksymacja (double)', 'punkty pomiarowe', 'Location', 'best');
    ...
    % Wykres 2: yv
    subplot(3,1,2);
    plot(x_fine, yv, 'r-', x, y, 'ko'); 
    title('Aproksymacja w precyzji vpa');
    xlabel('x (znormalizowany czas)');
    ylabel('Produkcja energii');
    legend('aproksymacja (vpa)', 'punkty pomiarowe', 'Location', 'best');
    ax = axis;
    ax(3) = ymin;
    ax(4) = ymax;
    axis(ax)
