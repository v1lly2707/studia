...
public class PrzewodnikStudencki extends Student {
    ...
    public void reagujNaAtrakcje(Atrakcja a, double czas) {
        if(a instanceof DrewniaCerkiew) {
            DrewniaCerkiew cerkiewka = (DrewniaCerkiew) a;
            mow("To jest drewniana cerkiew w miejcowości " + cerkiewka.getNazwaMiejscowosci() + ". Wiele taki można spotkać w Beskidach. Po mojej lewej widać wieżę o kontrukcji słupowo ramowej ...");
            regeneruj(czas);
        }

        else if(a instanceof CmentarzZIWojny) {
            CmentarzZIWojny cmentarz = (CmentarzZIWojny) a;
            mow("Tutaj widzimy cmentarz z I Wojny Światowej w miejcowości "+cmentarz.getNazwaMiejscowosci()+". Takie cmentarze projektował m.in. Dušan Jurkovič.");
            regeneruj(czas);
        }
        else {
