...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            """
            # TODO (0.5 point)
            x_data = x_data.dot(self.W) + self.b
            return self.f_act(x_data)
...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            x_data = self.hidden_layer.forward(x_data)
            return self.output_layer.forward(x_data)
            raise NotImplementedError()
    ...
    model.hidden_layer.W[:, 0] = [0, -1]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [0, 1]    # wagi neuronu h2
    model.hidden_layer.b[:] = [0.1, -1]         # biasy neuronów h1 i h2
